...
{
	utils::SetColor( Color4f{ r,g,b,a } );
}
...
bool utils::IsOverLappingPolygon(const std::vector<Point2f>& vertices, const Rectf& r)
{
	//Verify whether one of vertices is in rect
	for (size_t index{ 0 }; index < vertices.size(); ++index)
	{
		if (IsPointInRect(vertices[index], r))
		{
			return true;
		}
	}

	//Verify whether one of the polygon edges overlaps with rect
	for (size_t index{ 0 }; index < vertices.size(); ++index)
	{
		if (IsOverlapping(vertices[index], vertices[(index + 1) % vertices.size()], r))
		{
			return true;
		}
	}

	//No overlapping with edges, verify whether rect is completely inside polygon
	const Point2f rectCenter{ r.left + r.width / 2, r.bottom + r.height / 2 };
...
Point2f utils::PointOnLinearBezier(const LinearBezier& bez, float t)
{
	return PointOnLinearBezier(bez.p0, bez.p1, t);
}

Point2f utils::PointOnQuadraticBezier(const QuadraticBezier& bez, float t)
{
	return PointOnQuadraticBezier(bez.p0, bez.p1, bez.p2, t);
}
Point2f utils::PointOnCubicBezier(const CubicBezier& bez, float t)
{
	return PointOnCubicBezier(bez.p0, bez.p1, bez.p2, bez.p3, t);
}

Point2f utils::PointOnLinearBezier(const Point2f& p0, const Point2f& p1, float t)
{
	return Point2f{ (1 - t) * p0.x + t * p1.x,
					(1 - t) * p0.y + t * p1.y };
}

Point2f utils::PointOnQuadraticBezier(const Point2f& p0, const Point2f& p1, const Point2f& p2, float t)
{
	return Point2f{ powf((1 - t), 2) * p0.x + 2 * (1 - t) * t * p1.x + powf(t,2) * p2.x ,
					powf((1 - t), 2) * p0.y + 2 * (1 - t) * t * p1.y + powf(t,2) * p2.y };
}

Point2f utils::PointOnCubicBezier(const Point2f& p0, const Point2f& p1, const Point2f& p2, const Point2f& p3, float t)
{
	return Point2f{ powf((1 - t), 3) * p0.x
					+ 3 * powf((1 - t), 2) * t * p1.x
					+ 3 * (1 - t) * powf(t, 2) * p2.x
					+ powf(t, 3) * p3.x,
...
float utils::AngleRCubicBezier(const CubicBezier& bez, float t)
{
	return AngleRCubicBezier(bez.p0, bez.p1, bez.p2, bez.p3, t);
}

float utils::AngleDCubicBezier(const CubicBezier& bez, float t)
{
	return AngleDCubicBezier(bez.p0, bez.p1, bez.p2, bez.p3, t);
}

float utils::AngleRCubicBezier(const Point2f& p0, const Point2f& p1, const Point2f& p2, const Point2f& p3, float t)
{
	const Vector2f derivative{
		powf((1 - t), 2) * (p1.x - p0.x) + 2 * t * (1 - t) * (p2.x - p1.x) + powf(t, 2) * (p3.x - p2.x),
		powf((1 - t), 2) * (p1.y - p0.y) + 2 * t * (1 - t) * (p2.y - p1.y) + powf(t, 2) * (p3.y - p2.y)
	};

	const float angle{ atan2f(derivative.y, derivative.x) };

	return angle;
}

float utils::AngleDCubicBezier(const Point2f& p0, const Point2f& p1, const Point2f& p2, const Point2f& p3, float t)
{
	const float angleR{ AngleRCubicBezier(p0, p1, p2, p3, t) };

	return angleR * (180 / g_Pi);
}
